...
def process_frame(settings, Z, mean_brightness, nframes,
    ...
    if i == 0:
        io.imsave(os.path.join(save_folder, 'actual.jpg'),
                  np.array(255 * (labeled_removed == 0), dtype=np.uint8), check_contrast=False)
    ...
    if settings["skeletonize"]:
        ...
        if i == 0:
            io.imsave(os.path.join(save_folder, 'skeleton.jpg'),
                      np.array(255 * (skel_labeled == 0), dtype=np.uint8), check_contrast=False)
        ...
        for j in range(len(skel_props)):
            ...
            # ========================================================================
            # The following part was added and is not part of the original WF-NTP code
            # The added section saves the coordinates of the skeletons
            # in a .spine_unfiltered file
            # ========================================================================

            # Get the coordinates of the current skeleton
            coords = np.array(skel_props[j]['coords'])

            # Get 11 equally spaced points along the skeleton
            idx = np.linspace(0, len(coords) - 1, 11, dtype=int)
            spine_coords = coords[idx]

            # Save the 11 coordinates in a .spine file
            video_name = os.path.basename(settings["video_filename"]).split('.')[0]
            with open(os.path.join(save_folder, f'{video_name}.spine_unfiltered'), 'a') as f:
                date_time = os.path.basename(settings['save_as'])
                new_line = [date_time, '%05d' % (j), str(i)]  # i is the frame number

                for coords_pair in spine_coords:
                    new_line.extend([str(coords_pair[0]), str(coords_pair[1])])

                new_line.extend([str(skel_props[j]['centroid'][0]), str(skel_props[j]['centroid'][1])])
                f.write(' '.join(new_line) + os.linesep)
